...
import { HomeIcon, Plus, Icon, Filter, CalendarIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const stats = useMemo(() => ({
    total: tasks.length,
    lowPriorty: tasks.filter(t => t.priority?.toLowerCase() === 'low').length,
    highPriorty: tasks.filter(t => t.priority?.toLowerCase() === 'high').length,
    mediumPriorty: tasks.filter(t => t.priority?.toLowerCase() === 'medium').length,
  ...
  return (
    <div className={WRAPPER}>
      ...
      <div className={STATS_GRID}>
        {STATS.map(({ key, label, icon: Icon, iconColor, borderColor = "border-amber-100",
      ...
      <div className='space-y-6'>
        ...
        {/* ADD TASK DESKTOP */}
        <div className='hidden md:flex items-center justify-center p-4 border-2 border-dashed border-amber-200
        rounded-xl hover:border-amber-400 bg-amber-50/50 cursor-pointer transition-colors'>
