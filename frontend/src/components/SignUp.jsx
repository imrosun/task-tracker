...
import {Inputwrapper, FIELDS, BUTTONCLASSES, MESSAGE_ERROR, MESSAGE_SUCCESS } from '../assets/dummy.jsx'
...
const SignUp = ({onSwitchMode}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({text: err.response?.data?.message || "An error occured. Please try ahgain", type: "error"})
    } finally {
      setLoading(false)
    }
  }  
  ...
  return (
    <div className='max-w-md w-full bg-white shadow-lg border border-amber-100 rounded-xl p-8'>
      ...
      <p className='text-center text-sm text-gray-600 mt-6'>
        ...
        <button onClick={onSwitchMode} className='text-amber-600 hover:text-amber-700
