...
import { AlignLeft, Calendar, Flag, PlusCircle, Save, X } from 'lucide-react'
...
const TaskModal = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
    ...
    return (
        <div className='fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4'>
            <div className='bg-white border border-amber-100 rounded-xl max-w-md w-full shadow-lg relative 
        p-6 animate-fadeIn'>
                <div className='flex justify-between items-center mb-6'>
                    <h2 className='text-2xl font-bold text-gray-800 flex items-center gap-2'>
                        {taskData.id ? <Save className='text-amber-500 w-5 h-5' /> :
                            <PlusCircle className='text-amber-500 w-5 h-5' />}
                        {taskData.id ? 'Edit Task' : 'Create New task'}
                    </h2>

                    <button onClick={onClose} className=' p-2 hover:bg-amber-100 rounded-lg
                transition-colors text-gray-500 hover:text-amber-700'>
                        <X className='w-5 h-5' />
                    </button>
                </div>
                ...
                border border-red-100'>
                    ...
                    <div>
                        ...
                        <div className='flex items-center border border-amber-100 rounded-lg px-3 py-2.5
                        ...
                        transition-all duration-200'>
                            <input type="text" name="title" required value={taskData.title}
                                onChange={handleChange} className='w-full foxus:outline-none text-sm'
                    ...
                    <div>
                        <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                            <AlignLeft className='w-4 h-4 text-amber-500' /> Description
                        </label>
                    ...
                    <div>
                        <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-2'>
                            <Calendar className='w-4 h-4 text-amber-500' /> Status
                        </label>
                            ...
                            { val: 'No', label: 'In Progress' }
                                ...
                                <label key={val} className='flex items-center'>
                                    <input type='radio' name='completed' value={val} checked={taskData.completed === val}
                                        onChange={handleChange} className='h-4 w-4 text-amber-600 
                                        focus:ring-amber-500 border-gray-300 rounded' />
