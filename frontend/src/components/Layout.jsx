...
import { Clock, TrendingUp, CalendarClock, Circle } from 'lucide-react'
...
const Layout = ({ onLogout, user }) => {
    ...
    const StatCard = ({ title, value, icon }) => (
        <div className='p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-amber-100 hover:shadow-md
        transition-all duration-300 hover:border-amber-100 group'>
            <div className='flex items-center gap-2'>
                <div className='p-1.5 rounded-lg bg-gradient-to-br from-yellow-500/10 to-amber-500/10 
                group-hover:from-yellow-500/20 group-hover:to-amber-500/20'>
                    {icon}
                </div>
                <div className='min-2-0'>
                    <p className='text-lg sm:text-xl font-bold bg-gradient-to-r from-yellow-500 to-amber-600
    ...
    if (error) return (
        <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
            <div className='bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md'>
                ...
                rounded-lg text-sm font-medium hover:bg-red-200 transition-colors'>
                    Try Again
                </button>
            </div>
        </div>
    ...
    return (
        <div className='min-h-screen bg-gray-50'>
            ...
            <div className='ml-0 xl:ml-64 lg:ml-64 md:ml-16 p-3 pt-16 sm:p-4 md:p-4 transition-all duration-300'>
                <div className='grid grid-cols-1  xl:grid-cols-3 gap-4 sm:gap-6'>
                    <div className='xl:col-span-2 space-y-3 sm:space-y-4'>
                        <Outlet context={{ tasks, refreshTasks: fetchTasks }} />
                    </div>

                    <div className='xl:col-span-1 space-y-2 sm:space-y-6'>
                        <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-amber-100'>
                            ...
                            <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                                ...
                                h-3.5 sm:w-4 sm:h-4 text-amber-500' />} />
                                ...
                                h-3.5 sm:w-4 sm:h-4 text-yellow-500' />} />
                                <StatCard title='Completion Rate' value={`${stats.completionPercentage} %`} icon={<CalendarClock className='w-3.5
                                h-3.5 sm:w-4 sm:h-4 text-amber-500' />} />
                            </div>
                            ...
                            <div className='space-y-2 sm:space-y-3'>
                                <div className='flex items-center justify-between text-gray-700'>
                                    <span className='text-xs sm:text-sm font-medium flex items-centergap-1.5'>
                                        <Circle className='w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500 fill-amber-500' />
                                        Task Progress
                                    </span>
                                    <span className='text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 sm:px-2
                                    rounded-fll'>
                        ...
                        <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-amber-100'>
                            ...
                            <div className='space-y-2 sm:space-y-3'>
                                {tasks.slice(0,3).map((task) => (
                                    ...
                                    p-2 sm:p3 hover:bg-amber-50/50 rounded-lg transition-colors duration-200
