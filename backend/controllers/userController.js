...
export async function updateProfile(req, res) {
    ...
    try {
        ...
        if (exists) {
            return res.status(409).json({success: false, message: "Email already in use by other account"});
        }
...
export async function updatePassword(req, res) {
    ...
    if (!currectPassword || !newPassword || newPassword.length < 8) {
    ...
    try {
        ...
        const match = await bcrypt.compare(currectPassword, user.password);
        if (!match) {
